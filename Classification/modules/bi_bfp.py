...
"""
Bidirection Block Floating Point
For example: 
1/128, 2/128, 3/128, ...127/128, 1, 2, 3, 4, ..., 126, 127;
"""
class BlockBiFP(InplaceFunction):
    @staticmethod
    def forward(ctx, input, bits, add_noise=True, block=False, block_dim=2):
        ...
        with torch.no_grad():
            ...
            if x.abs().max() != 0:
                ...
                if block == False :
                    exp = torch.log2(x.abs().max()).ceil()
                    int_scale = torch.pow(2, exp.sub(bits - 1))
                    frac_scale = torch.pow(2, exp.sub(2*bits - 2))
                    ...
                    if block_dim == 2:
                        ...
                        max_exponent = max_entry.add(1e-32).log2().ceil().view(
                            [x.size(0), x.size(1), 1, 1])
                        int_scale= torch.pow(2, (max_exponent.sub(bits-1)))
                        frac_scale= torch.pow(2, (max_exponent.sub(2*bits-2)))
                    elif block_dim == 1 :
                        # {Out_Channel, for weight};
                        # {Batch,  for Activation};
                        max_entry = torch.max(torch.abs(x.view(x.size(0), -1)), 1)[0]
                ...
                if add_noise:
                    noise = x.clone().uniform_(-0.5, 0.5)
                else :
                    noise = x.clone().zero_()


                int_x = x.div(int_scale).add(noise).round().clamp(
                        maxneg, maxpos).mul(int_scale)
                frac_x = x.div(frac_scale).add(noise).round().clamp(
                        maxneg, maxpos).mul(frac_scale)
                boundary = x.div(int_scale).abs().ge(1)
                x = torch.where(boundary, int_x, frac_x)
        return x
...
class BlockBiFP_Grad(InplaceFunction):
    @staticmethod
    ...
    def backward(ctx, grad_outputs):
        ...
        grad_inputs = num2bbifp(grad_outputs, bits, add_noise, block, block_dim)
...
def num2bbifp(x, bits, add_noise=False, block=False, block_dim=2):
    qx = BlockBiFP().apply(x, bits, add_noise, block, block_dim)
    return qx

def bbifp_grad(x, bits, add_noise=False, block=False, block_dim=2):
    qx = BlockBiFP_Grad().apply(x, bits, add_noise, block, block_dim)
    return qx


def bfpf_bbifpb(x, bits_f = 16, bits_b=16, add_noise=False, block=False, block_dim=2):
    x = num2bfp(x, bits_f, add_noise, block, block_dim)
    x = bbifp_grad(x, bits_b, False, block, block_dim)
    return  x
