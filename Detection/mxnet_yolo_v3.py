...
def parse_args():
    ...
    parser.add_argument('--datapath', type=str, default='voc',
                        help='root path for dataset Now support voc.')
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        train_dataset = gdata.VOCDetection(root=args.datapath,
            splits=[(2007, 'trainval'), (2012, 'trainval')])
        val_dataset = gdata.VOCDetection(root=args.datapath,
            splits=[(2007, 'test')])
        val_metric = VOC07MApMetric(iou_thresh=0.5, class_names=val_dataset.classes)
    elif dataset.lower() == 'coco':
        train_dataset = gdata.COCODetection(root=args.datapath, splits='instances_train2014', use_crowd=False)
        val_dataset = gdata.COCODetection(root=args.datapath, splits='instances_val2014', skip_empty=False)
